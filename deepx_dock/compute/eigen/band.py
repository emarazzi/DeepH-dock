...
class BandDataGenerator:
    ...
    def _parse_band_conf(self):
        self.fermi_energy = self.band_conf.get("fermi_energy_eV", self.fermi_energy)
    ...
    def _parse_k_list_spell(self, spell: str):
        ...
        self.hsk_vector_list = [[None, None] for _ in range(self.k_path_quantity)]
        self.hsk_symbol_list = [[None, None] for _ in range(self.k_path_quantity)]
        for i, kpi in enumerate(_spell_lines):
            kpi = kpi.split()
            self.k_path_density_list[i] = int(kpi[0])
            self.hsk_vector_list[i][0] = np.array([float(kpi[1]), float(kpi[2]), float(kpi[3])])
            self.hsk_vector_list[i][1] = np.array([float(kpi[4]), float(kpi[5]), float(kpi[6])])
    ...
    def _gen_all_k_list(self):
        ...
        for i_path in range(self.k_path_quantity):
            ...
            _curr_path_ks = [start_hsk + (float(i) / (num_k - 1)) * (end_hsk - start_hsk) for i in range(num_k)]
        ...
        for i_path in range(self.k_path_quantity):
            self.hsk_distance_list[i_path + 1] = (
                self._cal_k_distance(self.hsk_vector_list[i_path][0], self.hsk_vector_list[i_path][1])
        ...
        for i_path, n_kp in enumerate(self.k_path_density_list):
            ...
            for i_k in range(1, n_kp):
                _dist += self._cal_k_distance(
                    self.kpoints_frac_list[_prev_n_kp + i_k - 1], self.kpoints_frac_list[_prev_n_kp + i_k]
                )
    ...
    def calc_band_data(
        ...
        """
        Calculate band data with optional ill-conditioned eigenvalue handling.
        ...
        ill_method : str or None
            Method for ill-conditioned eigenvalue handling.
            Options: None, 'window_regularization', 'orbital_removal'.
        ill_threshold : float or None
            Threshold for ill-conditioning detection.
        window_emin : float or None
            Minimum energy for window regularization (relative to Fermi, in eV).
        window_emax : float or None
            Maximum energy for window regularization (relative to Fermi, in eV).
        """
        ill_handler = None
        if ill_method and ill_method != "none":
        ...
        if sparse_calc:
            ...
            print(
                f"Sparse calculation with num_band={_num_band}, lowest_band_energy={_lowest_band_energy}, maxiter={_maxiter} ..."
            )
            kwargs = {
                "k": _num_band,
    ...
    def _correct_fermi_energy(self):
        ...
        if not self._is_metal:
            ...
            if homo_index < 0 or homo_index >= self.band_quantity - 1:
                print("Warn: The fermi energy is out of the energy range of calculated bands.")
                return
    ...
    def _calculate_occupation_stats(self):
        self.k_occupy_counts = np.sum(self.band_data < self.fermi_energy, axis=0)
        self.band_occupy_counts = np.sum(self.band_data < self.fermi_energy, axis=1)
...
class BandPlotter:
    ...
    def _load_band_data(self, band_data_file_path: str | Path):
        with h5py.File(band_data_file_path, "r") as hf:
            ...
            self.hsk_symbol_list = [[s.decode("utf-8") for s in v] for v in hf["hsk_symbol_list"][()]]
    ...
    def plot(self, Emin=-10.0, Emax=10.0, plot_format="png", dpi=300):
        ...
        plt.hlines(0.0, xmin, xmax, colors="black", linestyles="dashed", linewidth=0.7)
    ...
    def _get_decelerated_hsk_symbol(self):
        ...
        for i_path in range(1, self.k_path_quantity):
            if self.hsk_symbol_list[i_path][0] == self.hsk_symbol_list[i_path - 1][1]:
        ...
        greek_symbol_list = ["Gamma", "Delta", "Theta", "Lambda", "Xi", "Pi", "Sigma", "Phi", "Psi", "Omega"]
